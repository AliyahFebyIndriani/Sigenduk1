<?php
$ci =& get_instance();
$u_id = $_SESSION['user'] ?? null;
$id_desa_aktif = 0;
$error_db = "";

if ($u_id) {
    // Kita ambil seluruh baris user untuk melihat apa saja isinya
    $user_row = $ci->db->where('id', $u_id)->get('user')->row_array();
    
    if ($user_row) {
        // Cek apakah kolom id_desa_hook ada dan isinya berapa
        if (isset($user_row['id_desa_hook'])) {
            $id_desa_aktif = $user_row['id_desa_hook'];
        } else {
            $error_db = "Kolom 'id_desa_hook' tidak ditemukan di tabel user!";
        }
    } else {
        $error_db = "User dengan ID $u_id tidak ditemukan di database!";
    }
}

// Gunakan instance Eloquent untuk mencari berdasarkan ID Desa Hook
// $id_desa_aktif diambil dari variabel yang sudah kita temukan sebelumnya
$config = \App\Models\Config::find($id_desa_aktif);

// Jika ID tidak ditemukan, baru ambil baris pertama sebagai backup
if (!$config) {
