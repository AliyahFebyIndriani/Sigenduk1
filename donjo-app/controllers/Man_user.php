...
class Man_user extends Admin_Controller
{
    ...
    public function form($p = 1, $o = 0, $id = '')
    {
        ...
        $this->render('man_user/manajemen_user_form.blade.php', $data);
    }
    // Tambahkan fungsi ini di dalam class Man_user
    public function cek_dan_simpan_wilayah()
{
    // ✨ PERUBAHAN 1.1: Validasi AJAX Request
    // Pastikan request dari AJAX untuk keamanan
    ...
    // ✨ PERUBAHAN 1.2: Validasi Input Kode Desa
    // Cek apakah kode desa kosong
    if (empty($kode_desa)) {
        echo json_encode([
            'status' => 'error',
    ...
    // ✨ PERUBAHAN 1.3: Try-Catch Error Handling
    try {
        // Cek apakah kode desa sudah ada di tabel config
        $cek = $this->db->get_where('config', ['kode_desa' => $kode_desa])->row();

        if (!$cek) {
            // Jika tidak ada, tambahkan sebagai data baru
            $data = [
                'kode_desa' => $kode_desa,
                'nama_desa' => 'Desa Baru',
                'nama_kepala_desa' => '',
                'kode_pos' => '',
                // Tambahkan field wajib lainnya sesuai struktur tabel config
            ];
            
            $insert = $this->db->insert('config', $data);
            
            // ✨ PERUBAHAN 1.4: Response JSON Lebih Lengkap
            if ($insert) {
                echo json_encode([
                    'status' => 'success',
                    'type' => 'baru',
                    'message' => 'Kode desa baru berhasil ditambahkan'
                ]);
            } else {
                echo json_encode([
                    'status' => 'error',
                    'message' => 'Gagal menambahkan kode desa baru'
                ]);
            }
        } else {
            echo json_encode([
                'status' => 'success',
                'type' => 'ada',
                'message' => 'Kode desa sudah terdaftar'
            ]);
        }
    } catch (Exception $e) {
        // ✨ PERUBAHAN 1.5: Tangkap Error Database
        echo json_encode([
            'status' => 'error',
...
private function validasi_dan_simpan_kode_desa($kode_desa)
{
    // Skip jika tidak ada kode desa
    if (empty($kode_desa)) {
        return true;
    }
    ...
    if (!$cek) {
        // Simpan kode desa baru
        $data = [
            'kode_desa' => $kode_desa,
            'nama_desa' => 'Desa Baru - ' . $kode_desa,
            'nama_kepala_desa' => '',
            'kode_pos' => '',
        ...
        try {
            ...
            log_message('error', 'Gagal menyimpan kode desa: ' . $e->getMessage());
            return false;
        }
    }

    return true; // Kode desa sudah ada, skip
}
public function cek_dan_buat_config() {
    // Tangkap data dari AJAX
    $id_desa_hook = $this->input->post('id_desa_hook', TRUE);

    if (empty($id_desa_hook)) {
        echo json_encode(['status' => 'error', 'message' => 'Kode desa tidak boleh kosong.']);
        return;
    }

    // 1. Cek apakah kode desa sudah ada di tabel config
    // Catatan: Sesuaikan 'config' dengan nama tabel Anda, dan 'kode_desa' dengan nama kolom Anda
    $cek_config = $this->db->get_where('config', ['kode_desa' => $id_desa_hook])->row();

    if (!$cek_config) {
        // 2. Jika belum ada, buat data baru (INSERT)
        $data_baru = [
            'kode_desa' => $id_desa_hook,
            // Anda bisa menambahkan default value untuk kolom lain jika tabel config mewajibkannya, misalnya:
            // 'nama_desa' => 'Nama Desa Belum Diatur',
            // 'created_at' => date('Y-m-d H:i:s')
        ];
        
        $insert = $this->db->insert('config', $data_baru);

        if ($insert) {
            echo json_encode(['status' => 'baru']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Gagal menyimpan data ke config.']);
        }
    } else {
        // 3. Jika sudah ada, kembalikan status 'ada'
    ...
    public function insert()
{
    ...
    // ✨ PERUBAHAN 3.1: Validasi dan Simpan Kode Desa SEBELUM Form Validation
    $kode_desa = $this->input->post('kode_desa');
    if (!empty($kode_desa)) {
        if (!$this->validasi_dan_simpan_kode_desa($kode_desa)) {
            session_error('Gagal menyimpan kode desa baru');
    ...
    public function update($p = 1, $o = 0, $id = '')
{
    ...
    // ✨ PERUBAHAN 4.1: Validasi dan Simpan Kode Desa SEBELUM Form Validation
    $kode_desa = $this->input->post('kode_desa');
    if (!empty($kode_desa)) {
        if (!$this->validasi_dan_simpan_kode_desa($kode_desa)) {
            session_error('Gagal menyimpan kode desa baru');
